...
function getElementExtent(element: CardElement): {
  ...
  if (element.type === "line") {
    const x2 = element.x2 ?? element.x + Math.max(0, element.lineWidth || 152);
...
function calculateAutoCardSize(elements: CardElement[]): {
  ...
  return {
    width: Math.min(
      1200,
      Math.max(100, Math.ceil(extent.right + CARD_PADDING)),
    ),
    height: Math.min(
      800,
      Math.max(50, Math.ceil(extent.bottom + CARD_PADDING)),
    ),
  };
}
...
export default function EditorPage() {
          ...
          .then(async (res) => {
            if (res.ok) {
              ...
              if (lastConfig.success) {
                ...
                setLastPublishedFingerprint(configFingerprint(lastConfig.data));
              }
            }
          })
          .catch(() => {}),
  ...
  const handleElementMouseDown = (e: React.MouseEvent, el: CardElement) => {
    ...
    const point = getCanvasPoint(e.clientX, e.clientY);
    if (!point) return;
    const visualY = el.type === "text" ? el.y - (el.fontSize || 16) - 4 : el.y;
            ...
            <div className="relative">
              <button
                ...
                className={`flex h-7 items-center gap-1.5 rounded px-2 text-[9px] font-bold transition-colors ${
                  showSizeMenu || widthMode === "auto" || heightMode === "auto"
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 rounded-md bg-[#222028] p-0.5">
                        {(["auto", "manual"] as const).map((mode) => (
                          ...
                          >
                            {t(
                              mode === "auto" ? "common.auto" : "common.manual",
                            )}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="space-y-4 border-t border-[#2a2a32] p-3">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded border border-[#3a3a42] overflow-hidden relative">
                                <input
                                  type="color"
                                  value={
                                    firstSelectedElement.progressColor ||
                                    "#26a641"
                                  }
                                  onMouseDown={saveToHistory}
                                  onChange={(e) =>
                      ...
                      {firstSelectedElement.type === "calendar" && (
                        <div className="space-y-1">
                          ...
                          <select
                            value={
                              firstSelectedElement.calendarFormat || "both"
                            }
                            onFocus={saveToHistory}
                            onChange={(e) =>
                              updateSelected({
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="space-y-1">
                              ...
                              <input
                                ...
                                max={20}
                                value={
                                  firstSelectedElement.lineStrokeWidth ?? 2
                                }
                                onFocus={saveToHistory}
                                onChange={(e) =>
                                  updateSelected({
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                              ...
                              <input
                                type="color"
                                value={
                                  firstSelectedElement.shapeStrokeColor ||
                                  "#ffffff"
                                }
                                onMouseDown={saveToHistory}
                                onChange={(e) =>
                            ...
                            <div className="space-y-1">
                              ...
                              <input
                                ...
                                max={20}
                                value={
                                  firstSelectedElement.shapeStrokeWidth || 0
                                }
                                onFocus={saveToHistory}
                                onChange={(e) =>
                                  updateSelected({
